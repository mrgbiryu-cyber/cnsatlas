...
def build_page_owner_semantic_groups(page: dict[str, Any], assets: dict[str, Any]) -> tuple[list[dict[str, Any]], set[str]]:
    ...
    active_entries: list[list[dict[str, Any]]] = [
        atoms for entry_index, atoms in enumerate(merged_entries) if entry_index not in absorbed_entry_indices
    ]

    def extract_selector_label_controls(entries: list[list[dict[str, Any]]]) -> list[list[dict[str, Any]]]:
        extracted_entries: list[list[dict[str, Any]]] = []
        for atoms in entries:
            background_candidates = [
                atom
                for atom in atoms
                if str(atom.get("layer_role") or "") == "background_card"
                and 180.0 <= float((atom.get("visual_bounds_px") or {}).get("width") or 0.0) <= 260.0
                and 35.0 <= float((atom.get("visual_bounds_px") or {}).get("height") or 0.0) <= 70.0
            ]
            if not background_candidates:
                extracted_entries.append(atoms)
                continue

            remaining = list(atoms)
            created_any = False
            for background_atom in background_candidates:
                background_bounds = background_atom.get("visual_bounds_px") or make_bounds(0.0, 0.0, 1.0, 1.0)
                left_limit = float(background_bounds["x"]) + float(background_bounds["width"]) * 0.4
                top_limit = float(background_bounds["y"]) + float(background_bounds["height"]) * 0.55
                label_atom = next(
                    (
                        atom
                        for atom in remaining
                        if atom is not background_atom
                        and str(atom.get("layer_role") or "") == "text"
                        and mostly_contains(background_bounds, atom.get("visual_bounds_px") or make_bounds(0.0, 0.0, 1.0, 1.0))
                        and len(str(atom.get("text") or "").strip()) <= 8
                        and "구매하기" not in str(atom.get("text") or "")
                        and float((atom.get("visual_bounds_px") or {}).get("x") or 0.0) <= left_limit
                        and float((atom.get("visual_bounds_px") or {}).get("y") or 0.0) <= top_limit
                    ),
                    None,
                )
                if not label_atom:
                    continue
                extracted_entries.append([background_atom, label_atom])
                remaining = [
                    atom
                    for atom in remaining
                    if atom is not background_atom and atom is not label_atom
                ]
                created_any = True

            if created_any:
                if remaining:
                    extracted_entries.append(remaining)
            else:
                extracted_entries.append(atoms)
        return extracted_entries

    active_entries = extract_selector_label_controls(active_entries)

    def extract_cross_entry_selector_controls(entries: list[list[dict[str, Any]]]) -> list[list[dict[str, Any]]]:
        mutable_entries = [list(entry) for entry in entries if entry]
        created_entries: list[list[dict[str, Any]]] = []

        for entry_index, entry in enumerate(mutable_entries):
            background_atoms = [
                atom
                for atom in list(entry)
                if str(atom.get("layer_role") or "") == "background_card"
                and 180.0 <= float((atom.get("visual_bounds_px") or {}).get("width") or 0.0) <= 260.0
                and 35.0 <= float((atom.get("visual_bounds_px") or {}).get("height") or 0.0) <= 70.0
                and "구매하기" not in str(atom.get("text") or "")
            ]
            for background_atom in background_atoms:
                background_bounds = background_atom.get("visual_bounds_px") or make_bounds(0.0, 0.0, 1.0, 1.0)
                left_limit = float(background_bounds["x"]) + float(background_bounds["width"]) * 0.4
                top_limit = float(background_bounds["y"]) + float(background_bounds["height"]) * 0.55
                label_entry_index: int | None = None
                label_atom: dict[str, Any] | None = None
                for other_index, other_entry in enumerate(mutable_entries):
                    for candidate in other_entry:
                        if str(candidate.get("layer_role") or "") != "text":
                            continue
                        text_value = str(candidate.get("text") or "").strip()
                        if not text_value or len(text_value) > 8:
                            continue
                        candidate_bounds = candidate.get("visual_bounds_px") or make_bounds(0.0, 0.0, 1.0, 1.0)
                        if not mostly_contains(background_bounds, candidate_bounds):
                            continue
                        if float(candidate_bounds["x"]) > left_limit or float(candidate_bounds["y"]) > top_limit:
                            continue
                        label_entry_index = other_index
                        label_atom = candidate
                        break
                    if label_atom:
                        break
                if not label_atom or label_entry_index is None:
                    continue

                mutable_entries[entry_index] = [atom for atom in mutable_entries[entry_index] if atom is not background_atom]
                mutable_entries[label_entry_index] = [atom for atom in mutable_entries[label_entry_index] if atom is not label_atom]
                created_entries.append([background_atom, label_atom])

        return [entry for entry in mutable_entries if entry] + created_entries

    active_entries = extract_cross_entry_selector_controls(active_entries)

    def split_selector_label_text(entries: list[list[dict[str, Any]]]) -> list[list[dict[str, Any]]]:
        mutable_entries = [list(entry) for entry in entries if entry]

        def all_atoms() -> list[dict[str, Any]]:
            return [atom for entry in mutable_entries for atom in entry]

        def has_nearby_chevron(label_atom: dict[str, Any]) -> bool:
            label_bounds = label_atom.get("visual_bounds_px") or make_bounds(0.0, 0.0, 1.0, 1.0)
            label_y = float(label_bounds["y"])
            for atom in all_atoms():
                if str(atom.get("text") or "").strip() != ">":
                    continue
                atom_bounds = atom.get("visual_bounds_px") or make_bounds(0.0, 0.0, 1.0, 1.0)
                atom_y = float(atom_bounds["y"])
                if abs(atom_y - label_y) <= 12.0:
                    return True
            return False

        def has_below_value_row(label_atom: dict[str, Any]) -> bool:
            label_bounds = label_atom.get("visual_bounds_px") or make_bounds(0.0, 0.0, 1.0, 1.0)
            label_x = float(label_bounds["x"])
            label_y = float(label_bounds["y"])
            for atom in all_atoms():
                if atom is label_atom or str(atom.get("layer_role") or "") != "text":
                    continue
                text_value = str(atom.get("text") or "").strip()
                if not text_value or text_value == str(label_atom.get("text") or "").strip():
                    continue
                atom_bounds = atom.get("visual_bounds_px") or make_bounds(0.0, 0.0, 1.0, 1.0)
                atom_x = float(atom_bounds["x"])
                atom_y = float(atom_bounds["y"])
                if abs(atom_x - label_x) <= 10.0 and 8.0 <= atom_y - label_y <= 24.0:
                    return True
            return False

        created_entries: list[list[dict[str, Any]]] = []
        for entry in mutable_entries:
            for index, atom in enumerate(list(entry)):
                if str(atom.get("layer_role") or "") != "text":
                    continue
                text_value = str(atom.get("text") or "").strip()
                tokens = text_value.split()
                if len(tokens) != 2:
                    continue
                if tokens[1] not in {"선택", "입력", "보기"}:
                    continue
                font_size = float(((atom.get("text_style") or {}).get("font_size_max")) or 0.0)
                if font_size > 10.0:
                    continue
                if not has_nearby_chevron(atom) or not has_below_value_row(atom):
                    continue

                bounds = atom.get("visual_bounds_px") or make_bounds(0.0, 0.0, 1.0, 1.0)
                x = float(bounds["x"])
                y = float(bounds["y"])
                h = float(bounds["height"])
                label_width = max(estimate_text_width(tokens[0], font_size) + 6.0, 18.0)
                gap = 3.0
                value_x = x + label_width + gap
                value_width = max(float(bounds["width"]) - (label_width + gap), 12.0)

                bg_atom = {
                    **atom,
                    "id": f"{atom['id']}:selector_bg",
                    "text": "",
                    "subtype": "shape",
                    "layer_role": "small_asset",
                    "shape_kind": "rect",
                    "visual_bounds_px": make_bounds(x - 1.0, y - 1.0, label_width + 2.0, h + 2.0),
                    "shape_style": {
                        "fill": {"kind": "none"},
                        "line": {"type": "scheme", "value": "bg1", "resolved_value": "FFFFFF", "alpha": None, "width_px": 0.5},
                    },
                }
                label_atom = {
                    **atom,
                    "id": f"{atom['id']}:selector_label",
                    "text": tokens[0],
                    "visual_bounds_px": make_bounds(x, y, label_width, h),
                }
                value_atom = {
                    **atom,
                    "id": f"{atom['id']}:selector_value",
                    "text": tokens[1],
                    "visual_bounds_px": make_bounds(value_x, y, value_width, h),
                }

                entry[index] = value_atom
                created_entries.append([bg_atom, label_atom])
                break

        if created_entries:
            return mutable_entries + created_entries

        # Fallback: apply the same split even when the selector text is isolated in its own entry.
        flat_atoms = all_atoms()
        for entry in mutable_entries:
            for index, atom in enumerate(list(entry)):
                if str(atom.get("layer_role") or "") != "text":
                    continue
                text_value = str(atom.get("text") or "").strip()
                tokens = text_value.split()
                if len(tokens) != 2 or tokens[1] not in {"선택", "입력", "보기"}:
                    continue
                font_size = float(((atom.get("text_style") or {}).get("font_size_max")) or 0.0)
                if font_size > 10.0:
                    continue

                label_bounds = atom.get("visual_bounds_px") or make_bounds(0.0, 0.0, 1.0, 1.0)
                label_x = float(label_bounds["x"])
                label_y = float(label_bounds["y"])
                if not any(
                    str(other.get("text") or "").strip() == ">"
                    and abs(float((other.get("visual_bounds_px") or {}).get("y") or 0.0) - label_y) <= 12.0
                    for other in flat_atoms
                ):
                    continue
                if not any(
                    other is not atom
                    and str(other.get("layer_role") or "") == "text"
                    and abs(float((other.get("visual_bounds_px") or {}).get("x") or 0.0) - label_x) <= 10.0
                    and 8.0 <= float((other.get("visual_bounds_px") or {}).get("y") or 0.0) - label_y <= 24.0
                    for other in flat_atoms
                ):
                    continue

                x = float(label_bounds["x"])
                y = float(label_bounds["y"])
                h = float(label_bounds["height"])
                label_width = max(estimate_text_width(tokens[0], font_size) + 6.0, 18.0)
                gap = 3.0
                value_x = x + label_width + gap
                value_width = max(float(label_bounds["width"]) - (label_width + gap), 12.0)
    ...
    active_entries = split_selector_label_text(active_entries)
