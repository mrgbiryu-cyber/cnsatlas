(function (global) {
  ...
  function extractPicture(node, relTargets, zip, slidePath) {
    ...
    return zip.file(resolvedTarget || "") ? zip.file(resolvedTarget).async("uint8array").then(function (bytes) {
      return {
        image_rel_id: embed,
  ...
  async function buildIntermediateModelFromZip(zip, pptxName) {
    ...
    var presentationRelsXml = await zip.file("ppt/_rels/presentation.xml.rels").async("string");
    ...
    if (zip.file("ppt/theme/theme1.xml")) {
      var themeXml = await zip.file("ppt/theme/theme1.xml").async("string");
    ...
    for (var i = 0; i < slides.length; i += 1) {
      ...
      var slideXml = await zip.file(slideInfo.slide_path).async("string");
      ...
      var slideRelTargets = {};
      if (zip.file(slideRelPath)) {
        var slideRelXml = await zip.file(slideRelPath).async("string");
